...
namespace CityGen
{
    public class TrafficCar : MonoBehaviour
    {
        ...
        public GameObject NightLight = null;
        ...
        private Transform GetTransformWheel(string wheelName)
        {
            ...
            wt = FindObjectsOfType(typeof(GameObject)).Select(g => g as GameObject).Where(g => g.name.Equals(wheelName) && g.transform.parent.root == transform).ToArray();
        ...
        private WheelCollider SetWheelComponent(int w)
        {
            ...
            col.radius = wheel[w].TryGetComponent(out MeshFilter filter) ? filter.sharedMesh.bounds.size.z * wheel[w].transform.localScale.z * 0.5f * 0.92f : 0.4f;
            col.mass = 2000;
        ...
        {
            DayNight.TimeChanged += TimeChanged;
        }
        ...
        private void TimeChanged(bool isNight)
        {
        ...
        private void MoveCar()
        {
            ...
            for (int k = 0; k < 4; k++)
            {
                ...
                wheel[k].rotation = Quaternion.Lerp(wheel[k].rotation, _rot, 0.2f);
        ...
        private float FixedRaycasts()
        {
            ...
            if (rStop > 0 && speed < 2)
            {
                ...
                else if (hit.transform.TryGetComponent(out TrafficCar car))
                {
                    StatusCar st = car.status;
        ...
        */
        ...
        private float VerifyTraffic(int t, float mts = 12)
        {
